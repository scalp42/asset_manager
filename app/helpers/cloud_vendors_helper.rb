  ...
  def add_rs_fields(fields,asset)
    ...
    fields.each do |field|
      if field != ''
        case field
          when 'flavor'
            flavor = Field.create(:field_type_id => FieldType.first(:type_name => "Select Field").id ,:description => 'RS Flavor',:name => 'RS Flavor',:vendor_type => cloudVendor.cloud_vendor_type)
          ...
          when 'operating_system'
            image = Field.create(:field_type_id => FieldType.first(:type_name => "Select Field").id ,:description => 'RS Operating System',:name => 'RS Operating System',:vendor_type => cloudVendor.cloud_vendor_type)
          ...
          when 'public_ip'
            public = Field.create(:field_type_id => FieldType.first(:type_name => "Single Line Text Field").id ,:description => 'RS Public IP',:name => 'RS Public IP',:vendor_type => cloudVendor.cloud_vendor_type)
            asset.asset_screen.build(:field_id => public.id.to_s ,:asset_id => asset.id,:required => true)
          when 'private_ip'
            private = Field.create(:field_type_id => FieldType.first(:type_name => "Single Line Text Field").id ,:description => 'RS Private IP',:name => 'RS Private IP',:vendor_type => cloudVendor.cloud_vendor_type)
